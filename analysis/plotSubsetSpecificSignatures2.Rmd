...
signDat <- read_delim(file = paste0(basedir,
                                  "/data/GSEA/SubsetSpecificSelectedGenes2.txt"),
                    delim = "\t") %>%
...
template_gs <- c(
    "#### {{sig}} cut3\n",
    "```{r sig expr cut3 {{sig}}, fig.height=6, fig.width=8}\n",
...
```{r project sign DM cut2.5, fig.height=6, fig.width=8}
...
template_gs <- c(
    "#### {{sig}} cut2.5\n",
    "```{r sig expr DM cut2.5 {{sig}}, fig.height=6, fig.width=8}\n",
    "sign <- as.character('{{sig}}')",
    "signGenes <- signDat %>% dplyr::filter(signSubset == sign)",
    "sceSub <- sce[which(rownames(sce) %in% signGenes$geneID),]",
    "cntMat <- rowSums(t(as.matrix(
