...
### project sign umap cut 3 {.tabset}
```{r project sign cut3}
...
### project sign DM cut 3{.tabset}
```{r project sign DM cut3, fig.height=6, fig.width=8}
...
template_gs <- c(
    "#### {{sig}} cut3\n",
    "```{r sig expr DM cut3 {{sig}}, fig.height=6, fig.width=8}\n",
    "sign <- as.character('{{sig}}')",
    "signGenes <- signDat %>% dplyr::filter(signSubset == sign)",
    "sceSub <- sce[which(rownames(sce) %in% signGenes$geneID),]",
    "cntMat <- rowSums(t(as.matrix(
