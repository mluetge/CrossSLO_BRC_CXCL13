...
signDat <- read_delim(file = paste0(basedir,
                                  "/data/GSEA/SubsetSpecificSelectedGenes.txt"),
                    delim = "\t") %>%
...
template_gs <- c(
    "#### {{sig}}\n",
    "```{r sig expr {{sig}}, fig.height=6, fig.width=8}\n",
...
### project sign DM {.tabset}
```{r project sign DM, fig.height=6, fig.width=8}
...
template_gs <- c(
    "#### {{sig}}\n",
    "```{r sig expr DM {{sig}}, fig.height=6, fig.width=8}\n",
    "sign <- as.character('{{sig}}')",
    "signGenes <- signDat %>% dplyr::filter(signSubset == sign)",
    "sceSub <- sce[which(rownames(sce) %in% signGenes$geneID),]",
    "cntMat <- rowSums(t(as.matrix(
...
template_gs <- c(
    "#### {{sig}} cut 2\n",
    "```{r sig expr DM {{sig}} cut 2, fig.height=6, fig.width=8}\n",
    "sign <- as.character('{{sig}}')",
    "signGenes <- signDat %>% dplyr::filter(signSubset == sign)",
    "sceSub <- sce[which(rownames(sce) %in% signGenes$geneID),]",
    "cntMat <- rowSums(t(as.matrix(
