...
```{r feature plot FDCs}
selVec <- c("Pi16", "Fndc1")
genes <- data.frame(ensID=rownames(seuratSub)) %>% 
  mutate(gene = gsub(".*\\.", "", ensID))
selGenes <- data.frame(gene=selVec) %>% left_join(., genes, by="gene")

pList <- sapply(selGenes$ensID, function(x){
  p <- FeaturePlot(seuratSub, reduction = "umap", 
            features = x, 
...
avgHeatmap <- function(seurat, selGenes, colVecIdent, colVecCond=NULL,
  ...
  colVecScale <- colorRampPalette(c("#45628f", "#F7F7F7", "#de425b"))(30)
...
Idents(seurat) <- seurat$SLO_plus_clusterlabel
selMarker <- list(Chemokine = c("Cxcl13", "Cxcl12", "Ccl19", "Ccl21a"),
                  valMarker = c("Cr2", "Madcam1", "Tnfsf11"),
                  PRCMarker = c("Cd34", "Pi16", "Fndc1")
                  )
...
ordVec <- levels(seurat)[c(4,2,5,1,3,8,10,9,7,6,13,12,15,11,14)]
gapC <- c(5,10)
...
plotReducedDim(sce, "DiffusionMap", colour_by = "clusterLabel",
               shape_by = "SLO")
plotReducedDim(sce, "DiffusionMap", colour_by = "SLO", shape_by = "SLO")

seurat <- as.Seurat(sce)
DimPlot(seurat, reduction = "DiffusionMap", cols=colPal,
...
## for each SLO individually
lapply(unique(seurat$SLO), function(slo){
  seuratSub <- subset(seurat, SLO==slo)
  DimPlot(seuratSub, reduction = "DiffusionMap", cols=colPal,
        group.by = "clusterLabel",
        shuffle=T,pt.size=0.4)+
  theme_void() +
  ggtitle(paste0("DM ", slo))
})
