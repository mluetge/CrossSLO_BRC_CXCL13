...
colPal <- c(rcartocolor::carto_pal(name="Safe"), pal_futurama()(8))
```
...
seuratSub <- rerunSeurat3(seuratSub, res = c(0.4, 0.1, 0.25, 0.6))
...
seurat$clusterLabel[which(seurat$RNA_snn_res.0.25 %in% c("0", "9"))] <- "redPulpFibroblast"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.25 == "10")] <- "BCRC"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.25 == "4")] <- "adventitial"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.25 == "7")] <- "capsular"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.25 == "6")] <- "aFDC"

### label from re-embedding
fdc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.6 %in% c("2"))]
mrc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.6 == "0")]
tbrc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.6 %in%
                                    c("1", "4", "5"))]
dzfdc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.6 == "3")]
...
colPal <- c("#387890", "#427CB6", "#F9DF35", "#F7E77C", "#FF9933", "#941372",
            "#F7C27C", "#FF6666", "#389055", "#A0A0A0", "#A83539", "#CC6600")
names(colPal) <- c("redPulpFibroblast", "muralCells", "PRC", "intPRC",
                   "LZFDC", "MRC", "aFDC","TBRC", "capsular", "adventitial",
                   "BCRC", "DZFDC")
...
selMarker <- list(FDC = c("Xist","Sox9", "Fcer2a", "Fcamr", "Cr2"),
                  aFDC = c("Cox4i1","Atp5e"),
                  MRC = c("Madcam1", "Cxcl13", "Tnfsf11"),                  
                  DZFDC = c("Pdlim3", "Crym", "Cxcl12"),
                  TBRC = c("Bst1", "Glycam1", "Fmod","Ccl21a","Ccl19","Ifitm1"),
                  PRC = c("Pdgfrb", "Tlx1", "Eng", "Ly6a", "Wt1"),
                  mural = c("Rgs5", "Cnn1", "Acta2", "Mcam"),
                  adventitial = c("Cd34","Igfbp6", "Lum")
                  )
