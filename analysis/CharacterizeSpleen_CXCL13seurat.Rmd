...
fdc <- colnames(seuratSub)[which(seuratSub$originalexp_snn_res.0.6 %in% c("2"))]
mrc <- colnames(seuratSub)[which(seuratSub$originalexp_snn_res.0.6 == "0")]
tbrc <- colnames(seuratSub)[which(seuratSub$originalexp_snn_res.0.6 %in%
                                    c("1", "4", "5"))]
dzfdc <- colnames(seuratSub)[which(seuratSub$originalexp_snn_res.0.6 == "3")]
...
gapR <- data.frame(Grp=unique(selGenes$Grp)) %>% 
  left_join(.,grpCnt, by="Grp") %>% mutate(cumSum=cumsum(cnt)) 
ordVec <- levels(seurat)[c(8,5,3,2,9,1,7,4,6)]
