...
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 == "5")] <- "IFRC"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 %in% c("2","9"))] <- "TBRC"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 == "8")] <- "Cxcl9TBRC"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 %in% c("3", "10"))] <- "adventitial"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 %in% c("10"))] <- "DPTpos"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 %in% c("0", "4"))] <- "medRC1"
seurat$clusterLabel[which(seurat$RNA_snn_res.0.4 %in% c("7", "1"))] <- "medRC2"


### label from re-embedding
dzfdc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.4 %in% c("4", "0"))]
mrc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.4 == "3")]
lzfdc <- colnames(seuratSub)[which(seuratSub$RNA_snn_res.0.4 %in% c("2", "1"))]
...
colPal <- c("#0066CC", "#a9c8d6" ,"#CCCC00", "#FF6666", "#FF9933", "#660000",
            "#006633", "#A0A0A0", "#CC6600", "#5d5f5f")
names(colPal) <- c("medRC1", "medRC2", "IFRC", "TBRC", "LZFDC", "MRC", 
                   "Cxcl9TBRC", "adventitial", "DZFDC", "DPTpos")
...
selMarker <- list(LZFDC = c("Fcer2a", "Cr2", "Fcamr", "Coch", "Sox9"),                  
                  MRC = c("Cxcl13", "Madcam1", "Tnfsf11", "Hamp2"),
                  DZFDC = c("Pdlim3", "Cxcl12"),                  
                  TRC_TBRC = c("Grem1", "Prelp", "Ccl21a","Ccl19"),
                  medRC = c("Nr4a1", "Inmt", "Cxcl1", "Cxcl2","Ccl2", "Ccl7"),
                  Cxcl9TBRC = c("Cxcl10", "Cxcl9", "Irgm1", "Irf7"),
...
ordVec <- levels(seurat)[c(9, 10, 7, 2, 4, 3, 1,6, 5,8)]
