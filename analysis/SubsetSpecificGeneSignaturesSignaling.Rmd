...
selGO <- c("LZFDC.GO:0046890", "LZFDC.GO:0007179", "LZFDC.GO:0007249",
           "DZFDC.GO:0006631", "DZFDC.GO:0042063",
           "MRC.GO:0044344", 
           "TBRC.GO:0045765", "TBRC.GO:0035455",
           "PRC2.GO:0045765", "PRC2.GO:0035265")


GOconsDat <- do.call("rbind", GOcons) %>% rownames_to_column(., var="IDlab") %>% 
  filter(IDlab %in% selGO)
GOconsDat$subset <- factor(GOconsDat$subset, levels=c("PRC2", "TBRC", "MRC",
                                                      "DZFDC","LZFDC"))
GOconsDat <- GOconsDat %>% 
...
ggplot(GOconsDat, aes(x=factor(DescriptionLab, level= level_order),
                      y=geneRatio)) + 
