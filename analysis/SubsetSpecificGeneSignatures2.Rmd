...
```{r highly conserved makers, eval=FALSE, fig.height=8, fig.width=14, include=FALSE}
...
### vis summary all
```{r vis summary, fig.height=6, fig.width=12}

## table to select pathways
GOconsDat <- do.call("rbind", GOcons)
#write.table(GOconsDat, quote=F, row.names = T, col.names = T, sep= "\t",
#            file = paste0(basedir, "/data/GSEA/SubsetSpecificGO.txt"))
	

selGO <- c("LZFDC.GO:0050900","LZFDC.GO:0033627", "LZFDC.GO:0007229",
           "LZFDC.GO:0002443","LZFDC.GO:0002697", "LZFDC.GO:0030098",
           "LZFDC.GO:0008593",
           "DZFDC.GO:0097529", "DZFDC.GO:0001819", 
           "MRC.GO:0050900", "MRC.GO:0060326",
           "MRC.GO:0071674", 
           "TBRC.GO:0019221", "TBRC.GO:0006749", "TBRC.GO:0002460",
           "PRC2.GO:0031589", "PRC2.GO:0032635")


GOconsDat <- do.call("rbind", GOcons) %>% rownames_to_column(., var="IDlab") %>% 
  filter(IDlab %in% selGO)
GOconsDat$subset <- factor(GOconsDat$subset, levels=c("PRC2", "TBRC", "MRC", "DZFDC","LZFDC"))
GOconsDat <- GOconsDat %>% 
...
### vis summary sel
```{r vis summary sel, fig.height=6, fig.width=12}
...
selGO <- c("LZFDC.GO:0002443", "LZFDC.GO:0030098",
           "DZFDC.GO:0097529", 
           "MRC.GO:0050900", 
           "TBRC.GO:0019221", "TBRC.GO:0002460",
           "PRC2.GO:0031589", "PRC2.GO:0032635")
...
GOconsDat <- GOconsDat %>% mutate(qscore=-log(p.adjust, base=10)) %>% 
  mutate(Qscore=ifelse(qscore<5, qscore, 5)) %>% 
  mutate(DescriptionLab = paste0(subset, ": ", Description))
level_order <- unique(GOconsDat$DescriptionLab)

ggplot(GOconsDat, aes(x=factor(DescriptionLab, level= level_order), y=geneRatio)) + 
  geom_point(aes(col=subset, size=Count, alpha=one_minPadjust)) + 
  scale_color_manual(values = colPal)+
  scale_alpha_continuous(range = c(0.5,1), name="qvalue",
                         labels=c(0.04, 0.03, 0.02, 0.01),
  ...
  scale_size_continuous(range = c(3,6), labels = c(2,4,6,8),
                        breaks = c(2,4,6,8)) +
  geom_segment(aes(x=DescriptionLab, 
                   xend=DescriptionLab, 
                   y=min(geneRatio), 
                   yend=max(geneRatio)), 
               linetype="dashed", 
